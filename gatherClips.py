...
def searchYoutube(query,numResults):
	...
	request = youtube.search().list(q= query, part = 'snippet', type = 'video',maxResults =numResults) #(q = query, part = 'snippet', type = 'channel,video',maxResult = # you want to see)
...
def downloadVideos720p(urlQuery,clipCut):#downloads 720p video and audio individually to later be combined
	for url in urlQuery:
		prevVideos = os.listdir('videos') #prevents cropping videos that were previously cropped
		yt = YouTube(url)
...
def start():
	...
	clipCut = input('Lastly,when do you want the clip to end? "min:sec" NOTE: min>=1, sec<60 & Time cannot be longer than video  ')
	...
	while(delete):
		...
		for title in titles:
			...
			count+=1
		index = input("Delete any of the Suggestions?(# of entry you dont want included, 'n' if okay) ")
		if index == 'n':
			delete = False
			break
		else:
